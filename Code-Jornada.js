...
//
// sin_horarios=1 (Fase 1B) / sin_perfiles=1 (Fase 2A, ambas en
// docs/PLAN-SHEETS-API-DIRECTA.md en este repo): variantes aditivas para el
// agregador de croma-backend — evitan ejecutar getHorarios()/getPerfiles()
// acá cuando esas piezas ya se leen aparte, directo por Sheets API desde
// Node, para sacarlas de la cola de ejecuciones de GAS. Sin los parámetros,
// comportamiento y contrato 100% idénticos a antes — nadie más los manda.
...
function doGet(e) {
  ...
  // Cache key incluye sin_horarios/sin_perfiles para no mezclar variantes
  // recortadas con la completa (si no, una respuesta recortada podría
  // servirse cacheada para un pedido normal, o viceversa).
  if (accion === 'datos_portal_empleado') return _cacheableTextOutput('datos_portal_empleado|' + (e.parameter.empleado || '') + '|' + (e.parameter.sin_horarios || '') + '|' + (e.parameter.sin_perfiles || ''), function() { return getDatosPortalEmpleado(e); });
